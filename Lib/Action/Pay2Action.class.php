<?php
class Pay2Action extends Action{
       //在类初始化方法中，引入相关类库    
    ...
    public function refund_order2(){
        ...
        $notify_url   =C('alipay2.notify_url'); //服务器异步通知页面路径
        //$return_url   =C('alipay.return_url'); //页面跳转同步通知页面路径
        $seller_email =C('alipay2.seller_email');//卖家支付宝帐户必填
    ...
    function notifyurl(){
        ...
        if($verify_result) {
            ...
            if($explode_result_details[0]) {
                $order_list=M("order2")->where("trade_no='".$explode_result_details[0]."'")->find();
            }
           ...
           $update_data['status'] = '5';
            ...
            if($order_list && ($order_list['status']!='5')) {
                M("order2")->where("trade_no='".$explode_result_details[0]."'")->save($update_data);   
            }
    ...
    function returnurl(){
        ...
        if($_GET['trade_status'] == 'TRADE_FINISHED' || $_GET['trade_status'] == 'TRADE_SUCCESS') {
                ...
                $this->redirect(C('alipay2.successpage'));//跳转到配置项中配置的支付成功页面；
            }else {
                echo "trade_status=".$_GET['trade_status'];
                $this->redirect(C('alipay2.errorpage'));//跳转到配置项中配置的支付失败页面；
            }
        }else {
            //验证失败
