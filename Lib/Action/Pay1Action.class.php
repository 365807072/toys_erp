				...
				if($order_type==4) {
					...
					if($out_trade_no) {
						...
						$check_order = $model->table('baby_toys_order as o')
							...
							->field('o.id as order_id,o.invite_user_id,o.status,o.business_id,b.way,o.rent_day,o.balance_price,o.need_price,o.sell_price,o.service_price,o.user_id,b.is_card,b.number,o.card_id,o.deposit_price,o.change_order_id,b.service_number,o.battery_price,o.is_prize')
...
//										$update_data['state'] = '0';
									...
									if(($check_is_card==0) || ($check_is_card==9) ) {
										...
										if(($check_business_id==1625) || ($check_business_id==1627) || ($check_business_id==2253) || ($check_business_id==3028) || ($check_business_id==2255) )  {
                                            ...
                                            //查询邀请优惠两次配送
                                            if(($check_business_id==3166)||($check_business_id==3164)||($check_business_id==3201)||($check_business_id==3189)||($check_business_id==3231)||($check_business_id==3233)||($check_business_id==3242)){
...
//                                                }
							...
							if(($check_status=='1')){
								...
								foreach($each_order_res as $value){
									...
									if($each_card_id>0){
//										$new_card_id_arr[] = $each_card_id;//所有card_id
									}else{
...
//                echo $each_change_business_id_new;die;
								...
								//查询退还玩具解冻的押金
								if($each_change_business_id_arr){
									$tmp_deposit_live_change_res = $model->table('baby_toys_deposit')
										->where("account_type=3 and status=2 and user_id='$each_user_id'and card_id='0' and  business_id1 in ($each_change_business_id_arr)")
										->field("SUM(price) as live_price_change")
										->find();
									$tmp_deposit_live_price_change = $tmp_deposit_live_change_res['live_price_change'];
								}
								//需要交付的押金
...
//            var_dump($each_change_business_id_arr);die;
...
//													$model_business_listing->where("id=$getChange_toys_number")->save($upBusLisdata);
							...
							//需要交付的所有金额总计/除去配送费
							$need_pay_price = $need_deposit_price_total+$new_sell_price_total+$new_battery_price_total;
							//需要交付的所有金额总计
							$need_pay_price_total = $need_pay_price+$new_service_price_total;
